package com.zeml.rotp_spiderman.util;
...
public class SFBarriersNet {
	private Map<WebBarrierEntity, ShootingPoints> placedBarriers = new HashMap<>();
	private GraphAdjacencyList<Vector3d> closePoints = new GraphAdjacencyList<>();
	private double lastShotGap = -1;
    private boolean canShoot;

	public void tick() {
        Iterator<Map.Entry<WebBarrierEntity, ShootingPoints>> iter = placedBarriers.entrySet().iterator();
        while (iter.hasNext()) {
            WebBarrierEntity barrier = iter.next().getKey();
	...
	public void add(WebBarrierEntity barrier) {
		placedBarriers.put(barrier, generateShootingPoints(barrier));
		onUpdate();
	}
	
	private static final double SHOOTING_POINTS_GAP = 8;
	private ShootingPoints generateShootingPoints(WebBarrierEntity entity) {
	...
	private void onRemoved(WebBarrierEntity barrier) {
    ...
    public void shootStringsFromBarriers(IStandPower standPower, SpiderManEntity stand,
