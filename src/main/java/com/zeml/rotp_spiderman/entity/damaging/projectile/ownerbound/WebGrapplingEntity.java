...
public class WebGrapplingEntity extends OwnerBoundProjectileEntity {
    ...
    public WebGrapplingEntity(World world, StandEntity entity, IStandPower userStand) {
        super(InitEntities.SF_GRAPPLING_STRING.get(), entity, world);
        this.stand = entity;
        this.userStandPower = userStand;
    }
    
    public WebGrapplingEntity(EntityType<? extends WebGrapplingEntity> entityType, World world) {
    ...
    public void tick() {
        ...
        if (!level.isClientSide() && (userStandPower == null || userStandPower.getHeldAction() != (
        		bindEntities ? InitStands.SPIDER_MAN_GRAPPLE_ENTITY.get() :
        		    InitStands.SPIDER_MAN_GRAPPLE.get()))) {
        	remove();
        	return;
        }
        ...
        if (getEntityAttachedTo() == null && bindEntities) {
            if (target instanceof LivingEntity) {
                ...
                if (!JojoModUtil.isTargetBlocking(livingTarget)) {
                    ...
                    playSound(InitSounds.SPIDER_MAN_GRAPPLE_CATCH.get(), 1.0F, 1.0F);
            	    caughtAnEntity = true;
                    return true;
                }
            }
    ...
    private void placeBarrier() {
        ...
        if (!level.isClientSide() && !placedBarrier && getOwner() instanceof SpiderManEntity) {
            SpiderManEntity stonefree = (SpiderManEntity) getOwner();
